...
- 当前仓库未保留 scanner 专项自动测试文件；相关行为只被运行时、项目物化和严格校验链间接覆盖。
...
- 会因命中 `python_route_handler` / `python_route_builder` 被提升为高优先级候选
- 当前仓库不再把 `@governed_symbol` 注释作为正式绑定真相；正式绑定真相已经切到 registry 和 canonical 派生链
...
当前仓库没有单独保留 scanner 专项测试文件。

现有自动验证更偏向主链回归：

- [tests/test_project_runtime.py](../tests/test_project_runtime.py)
- [tests/test_knowledge_base_runtime.py](../tests/test_knowledge_base_runtime.py)
- [scripts/validate_strict_mapping.py](../scripts/validate_strict_mapping.py)

这意味着 scanner 文档中的若干“应命中 / 应失败”条目当前属于设计意图，而不是独立自动化测试承诺。
