...
## 3. 模块（最小可行基，Module）
- `M1` 竖向杆（`Module.VERTICAL_ROD`）：纵向主承力骨架
- `M2` 横向杆（`Module.HORIZONTAL_ROD`）：隔板支撑横梁
- `M3` 连接接口（`Module.CONNECTOR`）：对接关系，传力规则与装配语义
- `M4` 隔板（`Module.PANEL`）：提供承载面与把载荷分给连接点

## 4. 组合原则（Combination Principles）
- `R1`：组合不得孤立，至少包含 2 个模块（对应：`len(combo) >= 2`）
- `R2`：每个可用组合必须包含连接接口模块（对应：`Module.CONNECTOR in combo`）
- `R3`：一个置物架最小结构必须同时包含 `vertical_rod + horizontal_rod + connector + panel`
- `R4`：最小数量约束必须满足：
  - `vertical_rod >= 4`
  - `connector >= 4 * layers_n`
  - `horizontal_rod >= 4 * layers_n`
  - `panel >= layers_n`
- `R5`：在“1单位面积内多置物架”布局验证时，至少有一个置物架需满足 `layers_n >= 2`，以对齐“提升单位占地面积下的存取效率”目标
